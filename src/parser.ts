...
class JSONParser {
  ...
  #parseWsLetter<T extends JSONSymbolType>(
    type: T,
    letter: string,
  ): JSONSymbol<T> | null {
  ...
  parseObject(): JSONSymbol<'object'> | null {
    ...
    if (member === null) {
      ...
      if (endObject === null) {
        throw Error(
          `at ${this.index}, } expected, got ${this.text[this.index]}`,
        );
      }
    ...
    while (true) {
      ...
      if (valueSeparator === null) {
        ...
        if (member === null) {
          throw Error(
            `at ${this.index}, string expected, got ${this.text[this.index]}`,
          );
        }
        children.push(member);
      }
  ...
  parseMember(): JSONSymbol<'member'> | null {
    ...
    if (value === null) {
      throw Error(
        `at ${this.index}, value expected, got ${this.text[this.index]}`,
      );
    }
